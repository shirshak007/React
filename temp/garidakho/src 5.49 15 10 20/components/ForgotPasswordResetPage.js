...
export default function ForgotPassword(props) {
  ...
  const [token, setToken] = useState(props.match.params.token); //sending token with url
  const [email, setEmail] = useState(props.match.params.email); //sending email with url
  ...
  const page_details = async () => {
    var config = {
      method: "get",
      url: "http://garidakho.com/new_api/page_seo/?id=22",
      headers: {},
    };
  ...
  async function checkTokenMatchWithEmail() {
    var config = {
      method: "get",
      url:
        "http://garidakho.com/new_api/forget_password/?token_hash=" +
        token +
        "&email=" +
        email,
      headers: {},
    ...
    await Axios(config)
      .then(function (response) {
        if (response.data.success) setTokenMatch(true);
      })
      .catch(function (error) {
        console.log(error);
      });
  ...
  const handleClickSubmit = async () => {
    if (Password === ConfirmPassword && TokenMatch == true) {
