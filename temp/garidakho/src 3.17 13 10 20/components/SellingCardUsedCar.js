...
import ShowSingleUsedCar from "./ShowSingleUsedCar";
...
export default function SellingCard(props) {
  ...
  ---------------------------------------------------------------------------------------------------------------------*/
  const [Budget, setBudget] = React.useState([0, 1000000]); //This will be replaced by database values
...
*/
  ...
  const handleChangeBudgetMin = (newMinBudgetValue) => {
    //this function handles the changes in textfield  and reflects in slider
    let x = Number(newMinBudgetValue); //new value changed string to int
    let y = Budget[1]; //old value
    let z = [x, y]; //merged
    //console.log(z, Budget);
    setBudget(z); //set new value
  };
  const handleChangeBudgetMax = (newMaxBudgetValue) => {
    //this function handles the changes in textfield  and reflects in slider
    let x = Budget[0]; //old value
    let y = Number(newMaxBudgetValue); //new value changed string to int
    let z = [x, y]; //merged
    //console.log(z, Budget);
    setBudget(z); //set new value
  };
...
when new filter needed add before this/*
  ...
  const [SingleUsedCarId, setSingleUsedCarId] = React.useState("");
  const handleCloseSingleCar = () => {
    setOpenSingleCar(!OpenSingleCar);
  };
  const handleClickSingleCar = (SingleUsedCarId) => {
    setOpenSingleCar(!OpenSingleCar);
    setSingleUsedCarId(SingleUsedCarId);
  };

  /*This function resets all filters
  ----------------------------------------------------------------*/
  const handleClickClearAll = () => {
    // setBrand(""); //not working fix this
    setBudget([0, 1500000]);
    setKilometerDriven([0, 100000]);
    setModelYear([1990, new Date().getFullYear()]);
    setBodyType({
      Sedan: true,
      Coupe: true,
      Minivan: true,
  ...
  const getCarData = async () => {
    var config = {
      method: "get",
      url: "http://garidakho.com/new_api/gd_car_details/",
      headers: {},
    };
      ...
      <div className={classes.drawer}>
        ...
        <Collapse in={OpenBrand} timeout="auto" unmountOnExit>
          <List component="div" disablePadding>
            ...
            >
              {OpenBrand ? ( //this is the brand filter taking data from child component
                <CarDataAxios
                  newBrand={Brand}
                  onchangebrand={(e) => {
                    handleChangeBrand(e);
                ...
                >
                  {
                    ...
                    /*this data is taken reversely from child to parent*/
                  }
                </CarDataAxios>
              ) : null}
            </ListItem>
              ...
              <Grid container spacing={3}>
                <Grid item xs={12}>
                  <TextField
                    ...
                    label="Min"
                    value={Budget[0]}
                    onChange={(e) => handleChangeBudgetMin(e.target.value)}
                  />
                </Grid>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    label="Max"
                    value={Budget[1]}
                    onChange={(e) => handleChangeBudgetMax(e.target.value)}
                  />
                </Grid>
              </Grid>
            </ListItem>
        ...
        <List onClick={handleClickKilometerDriven}>
          <ListItem
            button
            style={{
              boxShadow: shadowKilometerDriven ? "" : "",
            }}
          >
            <ListItemIcon className={classes.icon}>
              <DriveEtaIcon />
        ...
        <List onClick={handleClickTransmissionType}>
          <ListItem
            button
            style={{
              boxShadow: shadowTransmissionType ? "" : "",
            }}
          >
            <ListItemIcon className={classes.icon}>
              <SettingsIcon />
        ...
        <List onClick={handleClickRegistrationCity}>
          <ListItem
            button
            style={{
              boxShadow: shadowRegistrationCity ? "" : "",
            }}
          >
            <ListItemIcon className={classes.icon}>
              <LocationCityIcon />
      ...
      <Container className={classes.container}>
        ...
        {OpenSingleCar ? (
          ...
          >
            ...
            <ShowSingleUsedCar id={SingleUsedCarId} />
          </Dialog>
        ) : null}
        <Grid container justify="center" spacing={5}>
          ...
          {CarData.map((car) => {
            if (
              ...
              Number(car.price) >= Budget[0] &&
              Number(car.price) <= Budget[1] &&
              Number(car.km_drive) >= KilometerDriven[0] &&
              Number(car.km_drive) <= KilometerDriven[1] &&
              Number(car.reg_year) >= ModelYear[0] &&
              Number(car.reg_year) <= ModelYear[1] &&
              (car.fuel_type === Fuel[0] ||
                car.fuel_type === Fuel[1] ||
                car.fuel_type === Fuel[2]) &&
              Number(car.owner) <= Owner &&
