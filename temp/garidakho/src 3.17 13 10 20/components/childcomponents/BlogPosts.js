...
export default function BlogMain() {
  ...
  const [LikeColor, setLikeColor] = React.useState(false); //manage liked post color
  ...
  //showing likes need fix
  const showLikes = async (NewId) => {
    var data = qs.stringify({
      blog_id: NewId,
    });
    var config = {
      method: "get",
      url: "http://garidakho.com/new_api/gd_blog_like/1?blog_id=" + NewId,
      headers: {
    ...
    await Axios(config)
      .then(function (response) {
        setNoOfLikes(response.data.total);
      })
          ...
          <Grid key={Blog.id} item xs={12} sm={6} md={6} lg={4}>
            <Card className={classes.cardroot}>
              <CardHeader
                ...
                title={Blog.blog_title_slug + " " + Blog.blog_title}
              ...
              <CardMedia
                onClick={() =>
                  handleClickSingleBlogPost(Blog.id)
                } /*The single post click event*/
                className={classes.media}
                image={"http://garidakho.com/new_api/images/" + Blog.blog_image}
                title={Blog.blog_description}
              >
                {/* if need custom image div, here the cardmedia has own image api */}
                {/* <img
                  className={classes.image}
                  src={"http://garidakho.com/new_api/images/" + Blog.blog_image}
                ...
                <Accordion>
                  ...
                  >
                    <Typography variant="body2">
                      {Blog.blog_description.slice(0, 40)}{" "}
                      <font style={{ color: "#6b6b6b" }}>...More</font>
                    </Typography>
                  </AccordionSummary>
                  <AccordionDetails>
                    <Typography>{Blog.blog_description}</Typography>
                  </AccordionDetails>
                </Accordion>
              </CardContent>
              <CardActions disableSpacing>
                <IconButton aria-label="add to favorites">
                  ...
                  <FavoriteIcon
                    style={{
                      color: LikeColor === true ? "red" : "",
                    }}
                  />
                </IconButton>
                {/* {NoOfLikes[Number(Blog.id)]} */}
                <IconButton
                  aria-label="comment box"
                  onClick={() => handleClickSingleBlogPost(Blog.id)} //if clicked open the full post
                >
                ...
                <IconButton
                  onClick={(e) => handleClickShare(e, Blog.id)} //open popup , passing post id for popup location
                  aria-label="share"
                >
                  <ShareIcon />
                </IconButton>
                  ...
                  <ListItem>
                    ...
                    <a
                      ...
                      href={
                        ...
                        " " +
                        BlogData[Number(Share)].blog_description
                      }
