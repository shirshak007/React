...
export default function SignIn() {
  ...
  const [cookies, setCookie] = useCookies(["token", "name", "id"]); //on successful login setcookie
  //initialization
    ...
    Axios(config)
      .then(function (response) {
        ...
        if (response.data.success) {
          console.log(response.data.post.name);
          setCookie("name", response.data.post.name, { path: "/" });
          setCookie("id", response.data.post.id, { path: "/" });
  ...
  const setCookies = async (Email, Password) => {
  ...
  const isLoggedIn = async () => {
    var config = {
      method: "get",
      url:
        "http://garidakho.com/new_api/cookie_data/?token='" +
        cookies.token +
        "'",
      headers: {},
    };
    await Axios(config)
      .then(function (response) {
        ...
        if (response.data.post.token == cookies.token) {
          history.push("/");
        }
      })
      .catch(function (error) {
