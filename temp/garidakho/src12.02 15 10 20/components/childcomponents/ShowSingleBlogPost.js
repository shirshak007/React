...
export default function ShowSingleBlogPost(props) {
  ...
  //getting cookies for like and comment purpose
  const [cookies, setCookie] = useCookies(["id"]);
  console.log(cookies.id, "hi blog" + props.id);
  ...
  /*handle like events
  -------------------------------------------------------*/
  const [LikeColor, setLikeColor] = React.useState(""); //manage liked post color
  ...
  const handleClickLike = async () => {
    ...
    var config = {
      method: "post",
      url:
        "http://garidakho.com/new_api/gd_blog_like/?user_id=" +
        cookies.id +
        "&blog_id=" +
        props.id,
      headers: {
      ...
      .then(function (response) {
        ...
        if (
          response.data.message ===
            String(
              "Blog like updated successfully"
            ) /*for clicking like multiple time*/ ||
          response.data.message === String("Blog Liked successfully") //for first time like
        ) {
          setLikeColor("red");
        } else if (
          response.data.message ===
          String(
            "Blog Disliked updated successfully"
          ) /*for clicking like multiple time*/
        ) {
          setLikeColor("");
  ...
  async function getIsLiked() {
    ...
    await Axios(config)
      .then(function (response) {
        console.log(response.data.post[0].islike);
        if (response.data.post[0].islike === "1") setLikeColor("red");
      })
      .catch(function (error) {
        console.log(error);
      });
