...
export default function SignIn() {
  ...
  const [cookies, setCookie] = useCookies(["name"]); //on successful login setcookie
  //initialization
  ...
  const UserLogin = async () => {
    ...
    Axios(config)
      .then(function (response) {
        ...
        setCookie("name", response.data.username, { path: "/" }); //setting the cookie, this name will be shown in home
        history.push("/");
      })
      .catch(function (error) {
        console.log(error);
